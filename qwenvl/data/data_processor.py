...
def _build_messages(item: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if isinstance(item, dict) and 'video_path' in item:
        ...
        item = {'conversations': [{'from': 'human', 'value': '<video>\nDescribe this video.'}, {'from': 'gpt', 'value': 'This is a testing prompt.'}]}
    elif "image" in item:
        video_pool = [{"type": "video", "video": os.path.join("/zehui/laion20M", item['image'])}]
        item = {'conversations': [{'from': 'human', 'value': '<video>\nDescribe this image.'}, {'from': 'gpt', 'value': 'This is a testing prompt.'}]}
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, processor, data_args):
        ...
        if data_args.add_image_data:
            ...
            # img_csv = "/home/v-yanboding/laion20M_merged_dedup.csv"
