        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            try:
                ...
                enable_multi_vq = False
            except:
                enable_multi_vq = True
                next_token_logits = [c_logits[:, -1, :].to(copy=True, dtype=torch.float32, device=input_ids.device) for c_logits in outputs.logits]

            # pre-process distribution
            if model_kwargs['task_type'] == "understanding":
                next_token_scores = logits_processor(input_ids, next_token_logits)
            elif model_kwargs["task_type"] == "generation":
            ...
            if do_sample:
                temperature = 0.98
                if isinstance(next_token_scores, list):
                    next_token_scores = [x / temperature for x in next_token_scores]
                    probs = [nn.functional.softmax(x, dim=-1) for x in next_token_scores]
                    next_tokens = torch.stack([torch.multinomial(p, num_samples=1).squeeze(1) for p in probs], dim=0)
                else:
                ...
                if isinstance(next_token_scores, list):
                    next_tokens = torch.stack([torch.argmax(x, dim=-1) for x in next_token_scores], dim=0)
                else:
            ...
            if model_kwargs["task_type"] == "generation":
                eos_token_id = next_token_logits[0].shape[-1] - 1
                eos_mask = input_ids[:, -1] == eos_token_id
                unfinished_sequences = unfinished_sequences & ~eos_mask
                if False:
                    eos_logit = next_token_logits[0, eos_token_id].item()
                    eos_prob = torch.softmax(next_token_logits[0], dim=-1)[eos_token_id].item()
                    print(f"[step {cur_len - first_cur_len + 1}] eos_logit={eos_logit:.3f}, eos_prob={eos_prob:.6f}")
                if (cur_len - first_cur_len) % 100 == 0 or all(eos_mask == True):
                    # import pdb; pdb.set_trace()
                    print(f"[step {cur_len - first_cur_len + 1}] [Multi VQ {enable_multi_vq}] token_id={next_tokens[-1].item()}")
                if cur_len - first_cur_len + 1 > 3510:
                    eos_logit = next_token_logits[0, eos_token_id].item()
                    eos_prob = torch.softmax(next_token_logits[0], dim=-1)[eos_token_id].item()
                    print(f"[step {cur_len - first_cur_len + 1}] eos_logit={eos_logit:.3f}, eos_prob={eos_prob:.6f}")
