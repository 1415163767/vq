        ...
        for shape, stacked_videos in grouped_videos.items():
            ...
            if do_resize:
                ...
                if kwargs['task'] == 'generation':
                    tgt_H, tgt_W = min([(512, 288), (384, 384), (288, 512)], key=lambda r: abs((r[1] / r[0]) - (W / H)))
                    scale = max(tgt_H / H, tgt_W / W)
                    new_H = max(int(np.ceil(H * scale)), tgt_H)
                    new_W = max(int(np.ceil(W * scale)), tgt_W)
                    frames = stacked_videos.permute(0, 1, 3, 4, 2).numpy()
                    resized_frames = []
                    for b in range(B):             # batch
                        for t in range(T):         # temporal
                            frame = frames[b, t]   # shape (H, W, C)
                            frame_resized = cv2.resize(frame, (new_W, new_H))
                            resized_frames.append(frame_resized)
                    resized_frames = np.stack(resized_frames, axis=0)  # (B*T, new_H, new_W, C)
                    resized_frames = resized_frames.reshape(B, T, new_H, new_W, C)
                    sh = (new_H - tgt_H) // 2
                    sw = (new_W - tgt_W) // 2
                    cropped_frames = resized_frames[:, :, sh:sh+tgt_H, sw:sw+tgt_W, :]
                    stacked_videos = torch.from_numpy(cropped_frames).permute(0, 1, 4, 2, 3)  # (B, T, C, tgt_H, tgt_W)
                    assert stacked_videos.shape[-2:] in [(512, 288), (384, 384), (288, 512)], \
                            f"Unexpected resolution: {stacked_videos.shape[-2:]}"
