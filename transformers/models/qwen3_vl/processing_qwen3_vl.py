    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen3VLProcessorKwargs],
        ...
        if videos is not None:
            output_kwargs["videos_kwargs"]['task'] = task
            if task == "generation":
                videos_inputs = self.video_processor(videos=videos, do_sample_frames=False, **output_kwargs["videos_kwargs"])
            else:
        ...
        if video_grid_thw is not None:
            ...
            for i in range(len(text)):
                while self.video_token in text[i]:
                    ...
                    # TODO: Whether add time tokens for generation task? If not, use 1D RoPE for video modeling.
                    if task == "generation":
                        token_t = int(video_grid_thw[index][0])
                        token_h = int(video_grid_thw[index][1]) // 2
                        token_w = int(video_grid_thw[index][2]) // 2
                        token_num = token_t * token_h * token_w
                        prefix = (
                            f"Generate a video with the following properties:\n"
                            f"- Number of tokens: {token_num}\n"
                            f"- Number of frames: {token_t}\n"
                            f"- Height: {token_h}\n"
                            f"- Width: {token_w}\n"
                            f"- Description: "
                        )
                        video_placeholder += (
                            self.vision_start_token + "<|placeholder|>" * frame_seqlen * video_grid_thw[index][0] + self.vision_end_token
                        )
                        text[i] = text[i].replace("<Replace>Replace Here<Replace>", prefix)
