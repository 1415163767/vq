...
class ProcessorMixin(PushToHubMixin):
    ...
    def apply_chat_template(
        ...
        task: Optional[str] = "understanding",
        ...
        **kwargs: Unpack[AllKwargsForChatTemplate],
        ...
        if task == "generation":
            for idx in range(len(conversations)):
                # for generation (text to video), reverse the source order
                conversations[idx] = conversations[idx][::-1]
                for content in conversations[idx]:
                    if content['role'] == 'assistant':
                        content['role'] = 'user'
                        content['content'][0]['text'] = "<Replace>Replace Here<Replace>" + content['content'][0]['text']
                    elif content['role'] == 'user':
                        content['role'] = 'assistant'
                        for dic_c in content['content']:
                            if dic_c['type'] == 'video':
                                content['content'] = [dic_c]
                                break
                    else:
                        raise ValueError(f"Unknown role {content['role']}")
                    
                    assert len(content['content']) == 1, "Only support single content for generation task."
        ...
        # Make sure 'video' is in the prompt
        if task == "generation" and "<|vision_start|><|video_pad|><|vision_end|>" not in prompt[0]:
